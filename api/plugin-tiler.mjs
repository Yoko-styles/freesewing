/**
 * Local copy of plugin-tiler for the export API container.
 * Adds a bottomRight fallback required by @freesewing/core@next's
 * Part.asRenderProps(), which crashes when bottomRight is false.
 * The original plugin-tiler.mjs is unchanged.
 */
...
const basePlugin = ({
  ...
  macros: {
    ...
    addPages: function (so, shorthand) {
      ...
      // @freesewing/core@next's Part.asRenderProps() requires bottomRight to be a Point.
      // When renderBlanks:false and the last tile has no content, the loop skips it and
      // bottomRight stays false. This fallback prevents the crash in the Docker container.
      if (!part.bottomRight) {
        part.width = cols * w
        part.height = rows * h
        part.bottomRight = new Point(cols * w, rows * h)
      }
